...
static __inline int atomic_int_get_suncc(volatile int *ptr)
{
...
static __inline void atomic_int_set_suncc(volatile int *ptr, int val)
{
    *ptr = val;
    __machine_rw_barrier();
}
...
static __inline int atomic_int_add_and_fetch_suncc(volatile int *ptr, int inc)
{
    return atomic_add_int_nv(ptr, inc);
}

#define avpriv_atomic_ptr_cas atomic_ptr_cas_suncc
static __inline void *atomic_ptr_cas_suncc(void * volatile *ptr,
